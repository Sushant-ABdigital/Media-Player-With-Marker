      ...
      function(_dereq_, module, exports) {
        ...
        Object.assign(MediaElementPlayer.prototype, {
          buildmarkers: function buildmarkers(player, controls, layers, media) {
            ...
            for (
              var i = 0, total = player.options.markers.length;
              i < total;
              ++i
            ) {
            ...
            media.addEventListener(
              ...
              function() {
                ...
                if (player.options.markers.length) {
                  for (
                    var _i = 0, _total = player.options.markers.length;
                    _i < _total;
                    ++_i
                  ) {
                    ...
                    if (
                      ...
                      currentMarker !== lastMarkerCallBack
                    ) {
          ...
          setmarkers: function setmarkers(controls) {
            ...
            for (var i = 0, total = t.options.markers.length; i < total; ++i) {
              if (
                ...
                Math.floor(t.options.markers[i]) >= 0
              ) {
                var left =
                    (100 * Math.floor(t.options.markers[i])) / t.media.duration,
                  marker = markers[i];

                marker.style.width = 20 + "px";
                marker.style.height = 20 + "px";
